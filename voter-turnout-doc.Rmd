...
## Young Voter Turnout and Different Race Groups
...
Could race be a factor in low turnout for young voters?
...
# TODO Summary statistics of race groups

# race_table <- race_df
# race_table <- na.omit(race_table)
# race_table %>% 
#  filter(Age == "18 to 24" & Age == "Total") %>% 
#  group_by(variable) %>% 
#  summarize(`Mean of Age Groups` = mean(value)) %>% 
#  rename(`Race Groups` = variable) %>% 
#  select(Year, Age, `Race Groups`)
```

Based on the plots above (Summary stats) Below is interactive.

## Interactive Plots on Voting Turnout and Race Groups
